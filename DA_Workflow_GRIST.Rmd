...
for (i in seq_along(search_terms)){
  relevant_tb <- eval(parse(text=str_c("`",search_terms[i],"`")))
  sorted_bar_chart <- top_x_PIs(relevant_tb,10) #default arguments = 10 PIs and for entire date range
}

for (i in seq_along(search_terms)){
  relevant_tb <- eval(parse(text=str_c("`",search_terms[i],"`")))
  sorted_bar_chart <- total_running_money_top_x_PIs(relevant_tb,10) #default arguments = 10 PIs and for entire date range
}
```
