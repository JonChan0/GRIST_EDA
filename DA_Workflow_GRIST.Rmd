...
The below code will answer the question of **who are the x most well-funded PI ?** (with arguments for top x as well as start date to filter) and **for the top x most well-funded PIs, what was the period of the grant & how much was each worth ?**.
...
top_x_PIs <- function(input_tb,x_to_keep=length(input_tb), end_date_cutoff = as.character(min(input_tb$`End Date`, na.rm=TRUE))){
  top_PIs <- input_tb %>%
...
top_x_PIs(epigenetics,10,end_date_cutoff = "2016-01-01")

total_running_money_calculator <- function(input_tb_rows){
  total_at_any_time <- vector("numeric",nrow(input_tb_rows))
  
  for (i in seq(1,nrow(input_tb_rows))){
    if (input_tb_rows[[2]][[i+1]] >= input_tb_rows[[3]][[i]]){ 
      #i.e if the start date of row i+1 comes after the end date of row i
      total_at_any_time[i] <- input_tb_rows[[4]][[i]]
    }
    
    if (input_tb_rows[[2]][[i+1]] < input_tb_rows[[3]][[i]]){
      #i.e if the start date of row i+ 1 comes before end date of row i.e overlapping grants
      total_at_any_time[i] <- input_tb_rows[[4]][[i]] + input_tb_rows[[4]][[i+1]]
    }
  }
}

# Stuck on how to create total_running_money - perhaps you should just do a nested list-column for each PI where you have dates by month and you if.... you add amount to get the total active at any time and if not ... you just have the amount.
...
total_running_money_top_x_PIs <- function(input_tb, x_to_keep = length(input_tb)){
  ...
  print(
    ggplot(top_x_amount)+
      geom_point(aes(`Start Date`,Amount,col=Name)) +
      geom_point(aes(`End Date`, Amount, col=Name)) +
      geom_step()
  )
  
}

total_running_money_top_x_PIs(CRISPR,10)
```
