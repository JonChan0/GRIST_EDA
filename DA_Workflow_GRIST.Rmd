...
top_x_PIs <- function(input_tb,x_to_keep=length(input_tb), end_date_cutoff = as.character(min(input_tb$`End Date`, na.rm=TRUE))){ # End date used as cutoff instead of start date because more results have NA as start date cf. end date.
  ...
  print(ggplot(top_x,aes(Total_Amount, reorder(Name, Total_Amount)))+
    ...
    labs(x="Total GBP Awarded",y="Name of PI", 
         title=str_c("Top",as.character(x_to_keep),"Well-Funded PIs for", str_to_title(top_x$Search_Term[[1]]), sep=" "), caption="Europe PMC GRIST API")
  )
  ...
  print(
    ggplot(top_x_amount)+
      ...
      labs(y="Total Running Amount of GBP Awarded",x="Year", 
         title=str_c("Running Grant GBP for Top",as.character(x_to_keep),"Well-Funded PIs for", str_to_title(top_x_amount$Search_Term[[1]]), sep=" "), caption="Europe PMC GRIST API")
  )
  return()
}
...
for (i in seq_along(search_terms)){
  relevant_tb <- eval(parse(text=str_c("`",search_terms[i],"`")))
  sorted_bar_chart <- top_x_PIs(relevant_tb,10,"2010-01-01") #default arguments = 10 PIs and for entire date range
}

for (i in seq_along(search_terms)){
  relevant_tb <- eval(parse(text=str_c("`",search_terms[i],"`")))
  sorted_bar_chart <- total_running_money_top_x_PIs(relevant_tb,10,"2010-01-01") #default arguments = 10 PIs and for entire date range
}
```
...
top_x_RIs <- function(input_tb,x_to_keep=length(input_tb), end_date_cutoff = as.character(min(input_tb$`End Date`, na.rm=TRUE))){ # End date used as cutoff instead of start date because more results have NA as start date cf. end date.
  ...
  print(ggplot(top_x,aes(Total_Amount, reorder(Institution, Total_Amount)))+
    geom_col() +
    labs(x="Total GBP Awarded",y="Name of Research Institution", 
         title=str_c("Top",as.character(x_to_keep),"Well-Funded RIs for", str_to_title(top_x$Search_Term[[1]]), sep=" "), caption="Europe PMC GRIST API")
  )
  ...
  print(
    ggplot(top_x_amount)+
      ...
      labs(y="Grant Award Amount /GBP",x="Year", 
         title=str_c("Grant Amounts for Top",as.character(x_to_keep),"Well-Funded RIs for", str_to_title(top_x_amount$Search_Term[[1]]), sep=" "), caption="Europe PMC GRIST API")
